...
void run_cat(char* option, char* name) {
	if(option == NULL) return;

	if(strcmp(option, ">") == 0 && name != NULL) {
		cat(structure, name, 1);
	}
	else if(strcmp(option, "-n") == 0 && name != NULL) {
		cat(structure, name, 2);
	}
	else {
		cat(structure, option, 0);
	}
}
